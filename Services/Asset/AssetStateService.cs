...
namespace WiseHR.Services
{
    public class AssetStateService
    {
        ...
        public async Task<AssetInstance?> GetAssetInstanceAsync(int instanceId)
        {
            try
            {
                _logger.LogInformation($"Fetching asset instance with ID {instanceId} from api/AssetIssueRecords?instanceId={instanceId}&include=assetinstance");
                Console.WriteLine($"Fetching asset instance with ID {instanceId}");

                var url = $"api/AssetIssueRecords?instanceId={instanceId}&include=assetinstance";
        ...
        public async Task CreateAssetIssueRecordAsync(AssetIssueRecord record)
        {
            try
            {
                _logger.LogInformation($"Creating issue record for instance ID {record.IssuedToEmployeeId} via api/AssetIssueRecords");
                ...
                _logger.LogInformation($"Created issue record for instance ID {record.IssuedToEmployeeId}");
        ...
        public async Task<bool> UnassignAssetIssueRecordAsync(AssetIssueRecord.UnassignRequest request)
        {
            try
            {
                _logger.LogInformation($"Unassigning issue record for instance ID {request.ReceivedByEmployeeId} via api/AssetIssueRecords/unassign");
                ...
                _logger.LogInformation($"Successfully unassigned issue record for instance ID {request.ReceivedByEmployeeId}");
                Console.WriteLine($"Successfully unassigned issue record for instance ID {request.ReceivedByEmployeeId}");
                return true;
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(ex, $"Network error unassigning instance: {ex.Message}");
                Console.WriteLine($"Network error unassigning instance {request.AssetInstanceId}: {ex.Message}");
                return false;
            }
            catch (JsonException ex)
            {
                _logger.LogError(ex, $"Deserialization error unassigning instance: {ex.Message}");
                Console.WriteLine($"Deserialization error unassigning instance {request.AssetInstanceId}: {ex.Message}");
                return false;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Unexpected error unassigning instance: {ex.Message}");
