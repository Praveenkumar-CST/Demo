...
namespace WiseHR.Services
{
    public class AssetStateService
    {
        ...
        public async Task DecommissionInstanceAsync(int instanceId)
        {
            try
            {
                _logger.LogInformation($"Decommissioning instance with ID {instanceId} via api/Assets/instances/{instanceId}/decommission");
                var response = await _httpClient.PutAsync($"api/Assets/instances/{instanceId}/decommission", null);
                string responseContent = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> UnAssignAssetIssueRecordAsync(UnassignRequest request)
        {
            try
            {
                ...
                _logger.LogInformation($"Unassigned issue record for instance ID {request.AssetInstanceId}");
                return true;
            }
            catch (HttpRequestException ex)
            {
                _logger.LogError(ex, $"Failed to unassign issue record for instance ID {request.AssetInstanceId} due to a network error.");
                Console.WriteLine($"HTTP error for unassigning issue record of instance {request.AssetInstanceId}: {ex.Message}");
                return false;
            }
            catch (JsonException ex)
            {
                _logger.LogError(ex, $"Failed to process unassign issue record response for instance ID {request.AssetInstanceId}.");
                Console.WriteLine($"Deserialization error for unassigning issue record of instance {request.AssetInstanceId}: {ex.Message}");
                return false;
            }
        }
    }
